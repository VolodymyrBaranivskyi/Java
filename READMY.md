...
###Use snake_case when naming symbols, methods, and variables. 
...
###Avoid single-line methods. And although they are quite popular among programmers, there are a lot of unpleasant little things connected with the syntax of their definition, which make application of such methods undesirable. In any case, single-line methods should not contain more than one expression. 

# badly
Def too_much; Something; Something_else; End

# Passable (note that the first `;` is mandatory)
Def no_braces_method; Body end

# Passable (note that the second `;` is optional)
Def no_braces_method; Body; End

# Passable (the correct syntax, but no `;` makes it difficult to read)
...
# OK
Temperance = Person.new ('Temperance', 30)
Parentheses can be omitted only in the following cases:
...
# Please note, elem is available outside of the loop
Elem # => 3

# OK
Arr.each {| elem | Puts elem}

# Elem is not available outside the block of each
...
# Good (assign `name` to 'Bozhidar' only if its
# The value of `nil` or` false`)
Name || = 'Bozhidar'
...
# better
Something && = something.downcase
###Avoid using the equality operator explicitly in case ===. As his name suggests, this statement is intended for implicit use in case expressions, in isolation from them, it only leads to different interpretations in the code. 
